...
export class Controller {
  ...
  btnFiltroInsideClick = async () => {
    ...
    const { points } = await this.map.getPoints()
    const pointsInsideMap = points.filter((point) => (this.checkPointInsideMap(point.x, point.y)))
    this.cleanPointsMap()
    this.renderPointsFilter(pointsInsideMap)
  }

  btnFiltroOutsideClick = async () => {
    ...
    const { points } = await this.map.getPoints()
    const pointsOutsideMap = points.filter((point) => (!this.checkPointInsideMap(point.x, point.y)))
    this.cleanPointsMap()
    this.renderPointsFilter(pointsOutsideMap)
  }

  checkPointInsideMap(x, y) {
    const MAP_START_X = this.map.boudingBox[0][0];
    const MAP_END_X = this.map.boudingBox[1][0];
    const MAP_START_Y = this.map.boudingBox[0][1];
    const MAP_END_Y = this.map.boudingBox[2][1];
    return ((x > MAP_START_X && x < MAP_END_X) && (y > MAP_START_Y && y < MAP_END_Y))
  }
  cleanPointsMap = () => {
    let pointsRendered = document.getElementsByClassName('map__point');
    while(pointsRendered.length > 0){
      pointsRendered[0].parentElement.removeChild(pointsRendered[0])
    }
  }
  ...
  async renderPointsFilter(points) {
    points.forEach(points => {
        this.drawPoint(points)
    })
  }
